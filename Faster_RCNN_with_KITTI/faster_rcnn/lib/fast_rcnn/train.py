...
class SolverWrapper(object):
    ...
    def train_model(self, sess, max_iters, restore=False):
        ...
        for iter in range(restore_iter, max_iters):
            ...
            total_loss_list.append(rpn_loss_cls_value + rpn_loss_box_value + loss_cls_value + loss_box_value)
            rpn_loss_cls_list.append(rpn_loss_cls_value)
            rpn_loss_box_list.append(rpn_loss_box_value)
            loss_cls_list.append(loss_cls_value)
            loss_box_list.append(loss_box_value)
        ...
        index_list = np.arange(max_iters)

        y_lists = []
        y_lists.append(total_loss_list)
        y_lists.append(rpn_loss_cls_list)
        y_lists.append(rpn_loss_box_list)
        y_lists.append(loss_cls_list)
        y_lists.append(loss_box_list)

        title_lists = []
        y_label_list = []
        title_lists.append("TOTAL LOSS TREND")
        title_lists.append("RPN CLS LOSS TREND")
        title_lists.append("RPN BOX LOSS TREND")
        title_lists.append("CLASS LOSS TREND")
        title_lists.append("BOX LOSS TREND")

        y_label_list.append("total loss")
        y_label_list.append("rpn class loss")
        y_label_list.append("rpn box loss")
        y_label_list.append("class loss")
        y_label_list.append("box loss")


        for p in range(len(y_lists)):
            # y to plot is jth column for the vector variance.
            y_list = y_lists[p]
            plt.subplot(2, 3, p + 1)
            plt.plot(index_list, y_list)
            plt.title(title_lists[p])
            plt.xlabel("iteration")
            plt.ylabel(y_label_list[p])

        plt.tight_layout()
        plt.savefig('kinds of losses.png')
        plt.show()

        plt.plot(index_list, total_loss_list)

        plt.plot(index_list, rpn_loss_cls_list)

        plt.plot(index_list, rpn_loss_box_list)

        plt.plot(index_list, loss_cls_list)

        plt.plot(index_list, loss_box_list)

        plt.title("LOSS TREND")

        plt.legend(["total loss", "rpn class loss", "rpn box loss", "class loss", "box loss"])
        plt.savefig('losses summary.png')
        plt.show()
